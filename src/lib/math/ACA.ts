...
export class ACA implements IApproximation {
    ...
    build(m: number, n: number, provider: IProvider, eps = 0.1) {
        ...
        console.log("REACHED the user precision. STOP the approximation with rank=", k)
        console.log("Usual  numbers: ", this.n_*this.m_)
        console.log("Approx numbers: ", this.k_*(this.n_+this.m_))
        console.log("Ratio         : ", ((this.n_*this.m_) / (this.k_*(this.n_+this.m_))).toFixed(2) )
        const percent = (100-(this.k_*(this.n_+this.m_))/(this.n_*this.m_)*100).toFixed(2)
    ...
    get n() {return this.n_}
    get m() {return this.m_}
    get rank() {return this.k_}
